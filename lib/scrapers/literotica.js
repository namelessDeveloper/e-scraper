...
export class Literotica {
  ...
  async #getAuthor(page) {
    const name = await page.$eval('.y_eU', getText)
    const href = await page.$eval('.y_eU', el => el.href)
    this.#authorAttributes.push(`author: "${name}"`)
    this.#authorAttributes.push(`authorUrl: "${href}"`)
    return `[${name}](${href})`
  }
