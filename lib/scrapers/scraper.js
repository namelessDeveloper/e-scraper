...
export class Scraper {
  ...
  async _promptLogin() {
    config()
    const envCredentials = process.env.milovana_user && process.env.milovana_pass
    if(envCredentials) {
      return {
        username: process.env.milovana_user,
        password: process.env.milovana_pass
      }
    }

    const username = new Input({
        name: 'username',
        message: 'Enter Milovana username'
    })

    const password = new Input({
        name: 'password',
        message: 'Enter Milovana password (this is only used to login)'
    })

    const name = await username.run()
    const pass = await password.run()
  ...
  async _fillInput(selectorString, text){
    await this._page.focus(selectorString)
